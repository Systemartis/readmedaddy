...
- **No network, ever, in shipped code.** The skill is Markdown; the hook, installer, wizard, and validator are POSIX shell and stdlib Python; the opencode notifier is a single dependency-free JS file — all doing local git and file reads. A **no-network guard** in [`scripts/validate-skill.py`](scripts/validate-skill.py) fails the build if any network primitive (`curl`, `wget`, `urllib`, `socket`, `requests.`, `/dev/tcp`, `fetch(`, `WebSocket`, …) appears in any shipped `.sh`, `.py`, or `.js` file.
...
- **Small enough to audit.** One shell hook, one installer, one wizard, one validator, two notify-only session hooks — read them before running; there are no dependencies, no build step, and no postinstall hooks to hide in. A **clean-for-publish guard** additionally screens every published file (Markdown, HTML, JS, config) in CI.
